...
describe('Testing /dm/list/v1', () => {
  ...
  test('Test-2, correct input parameters', () => {
    const user1 = requestAuthRegisterV2('test1@gmail.com', '1234567', 'Shenba', 'Chen').bodyObj;
    const user2 = requestAuthRegisterV2('test2@gmail.com', '123456', 'Richardo', 'Li').bodyObj;
    const DmCreate = requestDmCreateV1(user1.token, [user2.authUserId]);
  ...
  test('Test-3, calling dm list with different users', () => {
    ...
    expect(requestDmListV1(user1.token)).toStrictEqual(requestDmListV1(user2.token));
  });
});
...
describe('Testing /dm/details/v1', () => {
  ...
  test('Test-5, calling dm details with different users', () => {
    ...
    const user3 = requestAuthRegisterV2('test3@gmail.com', '12345678', 'Kunda', 'Yu').bodyObj;
    const dmId = requestDmCreateV1(user1.token, [user2.authUserId, user3.authUserId]).bodyObj.dmId;
    expect(requestDmDetailsV1(user1.token, dmId)).toStrictEqual(requestDmDetailsV1(user3.token, dmId));
  });
});
